...
import { BkDocument, localDocument } from '../document';
import { HoverDocumentGenerator } from '../utils/document-generator';
import { toKebabCase } from '../utils';
import { ExtensionConfigutation, ExtensionLanguage } from '../index';
...
export class ElementHoverProvier implements HoverProvider {
  ...
  provideHover(document: TextDocument, position: Position, token: CancellationToken): ProviderResult<Hover> {
    ...
    if (!/^[B|b]k/.test(tag?.text || '')) {
      // 如果不是element的标签(B|bk开头) 则返回 null 表示没有hover
      return null;
    }
  ...
  createHoverInstance(language: ExtensionLanguage, tag: string, attr: string, range: Range): null | Hover {
    ...
    if (Object.prototype.hasOwnProperty.call(document, tag)) {
      ...
      Object.keys(tagDocument).forEach((key: string) => {
        const hoverMarkdownString: MarkdownString = HoverDocumentGenerator.getInstance().generate<BkDocument>(tagDocument, key, tag, attr, language);
