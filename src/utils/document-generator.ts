...
import { ExtensionLanguage, ExtensionConfigutation } from '../index';
import { DocumentAttribute, BaseDocument, DocumentMethod, DocumentScopedSlot, DocumentSlot } from '../document';
...
export class HoverDocumentGenerator {
   ...
   */
  private generateAttribute<T extends BaseDocument>(document: T, tag: string, attribute: string, language: string, link?: string): MarkdownString {
    ...
    if (attributes.length) {
      // 生成表头
      if (language === 'zh-CN') {
        markdownString.appendMarkdown(`### [${tag}](${this.link}) 属性 \r`);
  ...
  private generateMethods<T extends BaseDocument>(document: T, tag: string, attribute: string, language: string): MarkdownString {
    ...
    if (methods.length) {
      if (language === 'zh-CN') {
        markdownString.appendMarkdown(`### [${tag}](${this.link}) 方法\r`);
        markdownString.appendMarkdown('| 方法 | 说明 | 参数 |\r');
      } else {
        markdownString.appendMarkdown(`### [${tag}](${this.link}) Method\r`);
  ...
  private generateEvents<T extends BaseDocument>(document: T, tag: string, attribute: string, language: string): MarkdownString {
    ...
    if (events.length) {
      if (language === 'zh-CN') {
        markdownString.appendMarkdown(`### [${tag}](${this.link}) 事件\r`);
        markdownString.appendMarkdown('| 方法 | 说明 | 参数 |\r');
      } else {
        markdownString.appendMarkdown(`### [${tag}](${this.link}) Event\r`);
  ...
  private generateSlots<T extends BaseDocument>(document: T, tag: string, attribute: string, language: string): MarkdownString {
    ...
    if (slots.length) {
      if (language === 'zh-CN') {
        markdownString.appendMarkdown(`### [${tag}](${this.link}) 插槽\r`);
        markdownString.appendMarkdown('| 插槽 | 说明 |\r');
      } else {
        markdownString.appendMarkdown(`### [${tag}](${this.link}) Slot\r`);
  ...
  private generateOther<T extends BaseDocument>(document: T, tag: string, attribute: string, language: string): MarkdownString {
    ...
    if (attributes.length) {
      markdownString.appendMarkdown(`### [${tag}](${this.link}) ${attribute} \r`);
  ...
  generate<T extends BaseDocument>(document: T, key: string, tag: string, attr: string, language: ExtensionLanguage): MarkdownString {
    ...
    const config = workspace.getConfiguration().get<ExtensionConfigutation>('magicbox-ui-helper');
    const _tag = tag.indexOf('bk-') > -1 ? tag.slice(3, tag.length) : tag;
    this.link = config?.link + _tag; // 生成文档地址
    switch (key) {
      case 'attributes':
